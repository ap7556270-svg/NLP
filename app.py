...
def ai_predict_label(statement: str, nlp) -> str:
    ...
    try:
        # Example: Use your best performing model (e.g., Logistic Regression)
        df_temp = pd.DataFrame({"statement": [statement]})
        X_feat, vect = apply_feature_extraction(df_temp["statement"], "Lexical & Morphological", nlp)
        model = LogisticRegression(max_iter=1000, solver='liblinear')
        # For simplicity, re-train quickly on your session data
        if not st.session_state.get('scraped_df', pd.DataFrame()).empty:
            df_train = create_binary_target(st.session_state['scraped_df'])
            df_train = df_train.dropna(subset=["target_label"])
            X_train, _ = apply_feature_extraction(df_train["statement"], "Lexical & Morphological", nlp)
